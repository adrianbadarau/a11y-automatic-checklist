...
2. The page is then passed to the **`A11yEvaluator`**, which prompts Gemini (the AI) up to 3 times in an iterative loop.
...
5. This loop repeats until Gemini says no important elements are un-badged, or the limit (3 iterations) is reached.
...
This feature runs automatically during check commands. 
